...
import ModalsComponents from '@/src/Components/core/Modal'
const Cases = () => {
        ...
        <div className='flex flex-wrap gap-4 md:gap-0 justify-center overflow-hidden'>
            {cases.map((item, index) => <div key={index} className='cases h-[500px] md:h-[600px] lg:h-[765px] md:w-6/12 lg:w-4/12 relative overflow-hidden'>
                ...
                <div className='absolute top-1/2  md:top-2/4 left-1/2  flex-col flex items-center justify-center ' style={{ transform: 'translate(-50% , -50%)', zIndex: 10 }}>
                    ...
                    <button className='uppercase text-[22px] px-5 py-2 bg-[rgba(125,196,200,0.2)] rounded-sm border border-white hover:border-[rgb(125,196,200)] hover:bg-[rgba(255,255,255,.2)] transition-all duration-500' onClick={handleOpen}>Explore</button>
                </div>

            </div>)}
            <ModalsComponents open={open} handleOpen={handleOpen} handleClose={handleClose} />
        </div>
    );
};
